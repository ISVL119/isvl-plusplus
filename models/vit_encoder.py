...
def _load_dinov3(name, repo_dir=None, weights=None, check_hash=False, force_reload=False):
    ...
    model = torch.hub.load(repo, name, **kwargs)
    model.eval()

    if hasattr(model, 'n_storage_tokens') and not hasattr(model, 'num_register_tokens'):
...
def load(name, repo_dir=None, weights=None, check_hash=False, force_reload=False):
        ...
        if "v2" in name:
            if "reg" in name:
                model = vision_transformer_dinov2.__dict__[f'vit_{arch}'](
                    patch_size=int(patchsize), img_size=518, block_chunks=0,
                ...
                model = vision_transformer_dinov2.__dict__[f'vit_{arch}'](
                    patch_size=int(patchsize), img_size=518, block_chunks=0,
                    init_values=1e-8, interpolate_antialias=False, interpolate_offset=0.1,
                )
                if arch == "base":
                    ckpt_pth = download_cached_file(
    ...
    if "digpt" in name:
        if arch == 'base':
            ...
            raise RuntimeError('Arch not supported in D-iGPT, must be base.')
