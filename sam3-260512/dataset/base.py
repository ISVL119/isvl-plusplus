...
class CPRDataset(Dataset):
    def __init__(self, dataset_name: str, sub_category: str, resize: int, data_dir: str, scales: List[int], region_sizes: List[int], retrieval_dir: str, foreground_dir: str = None, nAnomaly: int = 0, knn: int = 10) -> None:
        ...
        self._cache                   = {}
        self.root_dir                 = os.path.join('/home/hy/xsy_pan/dataset', self.dataset_name, sub_category)
        ...
        self.train_fns = [fn for fn in sorted(glob(os.path.join(self.root_dir, 'train/good/*'))) if os.path.isfile(fn)]
        self.test_fns = [
            fn for fn in (
                sorted(glob(os.path.join(self.root_dir, 'test_public/good/*'))) +
                sorted(glob(os.path.join(self.root_dir, 'test_public/bad/*')))
        ...
        if self.nAnomaly > 0:  # supervised
            _normal_data = []
            _outlier_data = []
            test_root = os.path.join(self.root_dir, 'test_public')
            for anomaly_name in sorted(os.listdir(test_root)):
                anomaly_dir = os.path.join(test_root, anomaly_name)
                if anomaly_name == 'ground_truth' or not os.path.isdir(anomaly_dir):
                    continue
                for fn in sorted(os.listdir(anomaly_dir)):
                    image_fn = os.path.join(anomaly_dir, fn)
            ...
            for image_fn in _outlier_data[:self.nAnomaly]:  # cache
                ...
                mask_fn = self.resolve_mask_fn(k)
        ...
        for image_fn in self.test_fns:
            ...
            mask_fn = self.resolve_mask_fn(k) if anomaly_name != 'good' else None
        ...
        if self.use_foreground and self.is_object:
            for info in tqdm(self.train_infos + self.test_infos + self.outlier_data, desc='load foreground', leave=False):
                ...
                image_stem = os.path.splitext(os.path.basename(k))[0]
                foreground_weight_path = os.path.join(
                    self.foreground_dir,
                    sub_category,
                    os.path.dirname(k),
                    f'f_{image_stem}.npy'
    ...
    def resolve_mask_fn(self, k: str):
        anomaly_name = os.path.dirname(k).rsplit('/', 1)[-1]
        if anomaly_name == 'good':
            return None
        image_name = os.path.splitext(os.path.basename(k))[0]
        candidates = [
            os.path.join(self.root_dir, 'test_public', 'ground_truth', anomaly_name, image_name + '_mask.png'),
            os.path.join(self.root_dir, 'test_public', 'ground_truth', anomaly_name, image_name + '.png'),
            os.path.join(self.root_dir, 'ground_truth', anomaly_name, image_name + '_mask.png'),
            os.path.join(self.root_dir, 'ground_truth', anomaly_name, image_name + '.png'),
        ]
    ...
    def __getitem__(self, idx):
        ...
        retrieval_image_fn = os.path.join('/home/hy/xsy_pan/dataset', self.dataset_name, sub_category, retrieval_k)
